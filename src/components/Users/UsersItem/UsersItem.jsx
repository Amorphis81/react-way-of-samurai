...
const UsersItem = props => {
  ...
  const followToggle = () => {
    return props.folowed ? <button disabled={props.followingInProgress.some(id => id === props.id)} onClick={() => props.unfollow(props.id)} className="btn users-list__btn">unfollow</button> :
                          <button disabled={props.followingInProgress.some(id => id === props.id)} onClick={() => props.follow(props.id)} className="btn users-list__btn">follow</button>
  }
  return (
